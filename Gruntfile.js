module.exports = function ( grunt ) {
  ...
  var taskConfig = {
    ...
    karma: {
      options: {
        configFile: '<%= build_dir %>/src/karma-unit.js'
      },
      unit: {
        port: 9019,
        background: true
        ...
        middleware: (function () {
          ...
          return function (connect, options) {
            ...
            middlewares.push(function (req, res, next) {
              ...
              var proxy;
              for (var i = options.proxies.length; i-- > 0;) {
                var contexts = options.proxies[i].context;
                contexts = Array.isArray(contexts) ? contexts : [contexts];
                for (var j = contexts.length; j-- > 0;) {
                  if (req.url.indexOf(contexts[j]) !== -1) {
                    proxy = {url: req.url, rule: options.proxies[i]};
                    break;
                  }
                }
              }
  ...
  grunt.registerTask( 'watch', [ 'onlybuild', 'delta' ] );
  ...
  grunt.registerTask( 'serv_bin', function () {
    grunt.task.run([
  ...
  grunt.registerMultiTask( 'karmaconfig', 'Process karma config templates', function () {
    ...
    grunt.file.copy( 'karma/karma-unit.tpl.js', grunt.config( 'build_dir' ) + '/src/karma-unit.js', { 
      process: function ( contents, path ) {
        return grunt.template.process( contents, {
          data: {
            scripts: jsFiles
